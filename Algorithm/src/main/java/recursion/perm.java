 ...
 */
    ...
    public static void permutation1(String str, String result, int len) {
        /* 全排列 递归实现
        递归思路：遍历，从第一个字母开始构建一个排列，添加第一字母，添加第二个字母，添加第三个字母
                result ""-->""+str.charAt(i)-->""+str.charAt(i)+str.charAt(j)
                每次添加的都是result中不存在的字符，直到result长度与原字符串长度一样
                （字符串中不能有重复字符）
      递归树：
          str:          a            b                c
                      ab ac         ba   bc         ca   cb
        ...
        if (result.length() == len) {            //表示遍历完了一个全排列结果
            ...
            for (int i = 0; i < str.length(); i++) {
                if (result.indexOf(str.charAt(i)) < 0) {    //indexOf返回指定字符在此字符串中第一次出现处的索引。
